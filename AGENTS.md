...
- **Python (uv)** does not edit `~/.bashrc` (`UV_NO_MODIFY_PATH=1`). `uv` and `python` land in `~/.local/bin`, which `shell/shared/env.sh` already prepends. After `10-python.sh`, open a new shell (or `exec bash`) before they resolve, unless that directory is already on `PATH`.
